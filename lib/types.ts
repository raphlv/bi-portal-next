export interface EconomicIndicator {
  ...
  unit: string;
  change: string;
  isPositive: boolean; // positive in economic context or up/down
  period: string;
  description: string;
...
export interface GovernorMember {
  name: string;
  role: string;
  period: string;
  photo: string;
...
export interface AgendaItem {
  id: string;
  title: string;
  type: 'Rapat Dewan Gubernur (RDG)' | 'Rilis Data Statistik' | 'Webinar / Sosialisasi' | 'Konferensi Pers';
  date: string;
  time: string;
  location: string;
